...
SELECT *
FROM range(3) AS src(key)
WHERE NOT EXISTS ( --We can use Exist or Not Exists depending on our need
    SELECT 1
    FROM range(2) AS tgt(key)
...
-- Final Example
-- Identify job postings that have no associated skills before loading them into a data market
SELECT * 
FROM job_postings_fact
ORDER BY job_id
LIMIT 10;
