...
WITH skills AS (
    ...
    UNION ALL
    SELECT 'r'
), skills_array AS ( --second WITH
    SELECT ARRAY_AGG(skill ORDER BY skill) AS skills -- ARRAY_AGG converts to array
    FROM skills
)
SELECT
...
WITH skill_table AS (
    ...
    UNION ALL
    SELECT 'r', 'programming'
), skills_array_struct AS (--starts second CTE 
    SELECT
        ARRAY_AGG( --using to create array of structs
            STRUCT_PACK( 
                skill := skills,
                type := types
            )
...
SELECT --showcasing how to access elements of array of structs
...
-- MAP
WITH skill_map AS (
    SELECT MAP {'skill' : 'python', 'type' : 'programming'} AS skill_type -- creating map / dictionary
)
SELECT
    skill_type['skill'],
    skill_type['type']
...
SELECT
    '{"skill":"python", "type":"programming"}'::JSON AS skill_json -- TO_JSON() is other way
)
SELECT
    STRUCT_PACK( -- converting JSON into STRUCT
...
SELECT -- Converting to an array of structs
    ARRAY_AGG(
        STRUCT_PACK(
...
-- Build a flat skill table for co-workers to access job title, salary info, and skills in one table
...
SELECT
    ...
    jpf.job_title_short,
    jpf.salary_year_avg,
    ARRAY_AGG(sd.skills) AS skills_array --grouping into an array to avoid duplicate rows by skill
